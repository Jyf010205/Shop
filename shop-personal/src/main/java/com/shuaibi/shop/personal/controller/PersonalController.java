...
public class PersonalController {
    ...
    @PutMapping
    @ApiOperation("修改个人信息")
    public CommonResult<UpdateUserRequest> update(@Valid @RequestBody UpdateUserRequest request,
                                     @ApiIgnore @UserId String userId){
        Integer status = personalService.update(request,userId);
        if (status != 1){
            Asserts.fail("修改失败");
        }
        return CommonResult.success(request,"修改成功");
    }
